...
FROM
    ...
    select count(*) as role_mapping_counts
    from role_mapping ;
...
WHERE 
    ...
    AND year = (SELECT MAX(year) FROM movie);
...
WITH actress_ratings AS (
    SELECT n.name AS actress_name,
           ...
           ROW_NUMBER() OVER (ORDER BY SUM(r.avg_rating * r.total_votes) DESC) AS actress_rank
    FROM names AS n
...
SELECT 
    ...
    CASE
        WHEN AVG(ratings.avg_rating) > 8 THEN 'Superhit'
        WHEN AVG(ratings.avg_rating) BETWEEN 7 AND 8 THEN 'Hit'
        WHEN AVG(ratings.avg_rating) BETWEEN 5 AND 7 THEN 'One-time-watch'
        WHEN AVG(ratings.avg_rating) < 5 THEN 'Flop'
